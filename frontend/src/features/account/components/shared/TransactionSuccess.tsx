...
interface TransactionSuccessProps {
  ...
  onReturn: () => void;
  reserveName?: string; // 🔹 nuevo campo opcional
}
    ...
    <Card className="w-full max-w-md p-6 bg-white rounded-lg shadow-xl transform transition-all scale-100 ease-out duration-300">
      <div className="text-center space-y-4">
        ...
        <div className="text-left space-y-2 bg-gray-50 p-4 rounded-lg border border-gray-100">
          <p>
            <span className="font-medium text-gray-700">N° Transacción:</span>{' '}
            <span className="text-gray-900">{transactionDetails.transactionId}</span>
          </p>
          <p>
            <span className="font-medium text-gray-700">Fecha:</span>{' '}
            <span className="text-gray-900">
              {format(new Date(transactionDetails.transactionDate), 'PPpp')}
            </span>
          </p>
          <p>
            <span className="font-medium text-gray-700">Desde:</span>{' '}
            <span className="text-gray-900">
              {type === 'RELEASE' ? reserveName || reason : userName}
            </span>
          </p>
          <p>
            <span className="font-medium text-gray-700">Hacia:</span>{' '}
            <span className="text-gray-900">{destinationAccountName}</span>
          </p>
        ...
        <Button
          onClick={onReturn}
          fullWidth
          className="mt-6 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg focus:outline-none focus:shadow-outline transition-colors duration-200"
        >
          Volver al dashboard
        </Button>
      </div>
