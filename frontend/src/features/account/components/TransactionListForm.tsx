...
const TransactionListForm = ({ sourceAccountId }: Props) => {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter(tx =>
      ...
      (tx.destinationAccount && tx.destinationAccount.toLowerCase().includes(lowered)) ||
      (tx.reason && tx.reason.toLowerCase().includes(lowered))
    );
      ...
      <Card className="w-full max-w-full">
        ...
        <div className="w-full flex justify-between items-center mb-3 mt-1 px-3">
          <div>
            ...
            <p className="text-slate-500">Resumen de los movimientos de la cuenta seleccionada.</p>
          </div>
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-[980px] w-full text-left table-auto border-collapse">
            <thead>
              <tr className="bg-slate-50">
                <th className="p-4 border-b border-slate-200 text-sm text-slate-500">ID</th>
              ...
              {pagedTransactions.map(tx => {
                ...
                return (
                  <tr key={tx.transactionId} className="hover:bg-slate-50 border-b border-slate-200">
                    <td className="p-4 py-5 text-sm text-slate-700 font-medium">{tx.transactionId}</td>
                    <td className="p-4 py-5 text-sm text-slate-500">{new Date(tx.transactionDate).toLocaleString()}</td>
                    <td className="p-4 py-5 text-sm text-slate-500">{typeLabel}</td>
                    <td className="p-4 py-5 text-sm text-slate-700">
                      <div className="flex items-center space-x-2">
                        <span className="text-sm text-slate-700">{amountText}</span>
                        {arrow}
                      </div>
                    </td>
